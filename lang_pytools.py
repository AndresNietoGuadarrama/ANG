...
def es_error_RAE(pal):
    '''
    Devuelve False si la palabra está en el dicionario de la RAE y True si no está (posible error)
    '''
    url="https://dle.rae.es/?w="+pal
    r=requests.get(url)
    texto=r.text
    i=texto.find('<meta name="description" content=')
    if texto[i+33:i+52]=='"Versión electrónic':
        return True
    else:
        return False
...
def es_error(frase):
    ...
    l_frase=frase.split(" ")
    for p in l_frase:
        if len(check(p, api_url, lang='es')['matches'])>0:
            return True
            break
    return False

def corrige_error(pal,med):
    '''
    Devuelve una posible corrección de la palabra.
    
    med: Catálogo de medicamentos.
    '''
    api_url='https://languagetool.org/api/v2/'
    response_dict=check(pal, api_url, lang='es')
    if (len(response_dict['matches'])>0 and len(response_dict['matches'][0]['replacements'])>0) and (pal not in med):
        return response_dict['matches'][0]['replacements'][0]['value']
    else:
        return pal
...
def corrector(args):
    ...
    list_t=t.split(" ")
    resp=""
    for j,pal in enumerate(list_t):
        if pal == "asignologico":
            pal_corregida="asignológico"
        elif pal == "asignologica":
            pal_corregida="asignológica"
        elif pal == "asignologicas":
            pal_corregida="asignológicas"
        elif pal == "asignologicos":
            pal_corregida="asignológicos"
        else:
            pal_corregida=corrige_error(pal,med)
        if j<len(list_t)-1:
            resp+=pal_corregida+" "
        else:
            resp+=pal_corregida
